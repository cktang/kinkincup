var slanskyKarlson = Backbone.Model.extend({
	base: [
		["AA","0.5","AA"],
		["KK","0.9","KK"],
		["AKs","1.2","KK","AKs"],
	...
	initialize: function() {
		this.baseMap = {};
		var self = this;
		_(this.base).each(function(list) {
			self.baseMap[list[0]] = parseFloat(list[1]);
		})
	},

	handsInRange: function(range) {
		return _(this.baseMap).chain().pairs().filter(function(p) { return p[1] <= range; }).pluck(0).value();
	},

	isHandInRange: function(hand, range) {
		if (_.isArray(hand)) hand = this.convertHand(hand);
		return this.findRange(hand) <= range;
	},

	findRange: function(hand) {
		if (_.isArray(hand)) hand = this.convertHand(hand);
		return this.baseMap[hand] || -1;		
	},

	convertHand: function(hand) {
		var type = _(hand).chain().map(function(c) { return c[1]; }).uniq().value().length == 1? "s": "";
		var strength = _(hand).chain().map(function(c) { return c[0]; }).sortBy(function(c) { return "AKQJT98765432".indexOf(c) }).value().join("");
